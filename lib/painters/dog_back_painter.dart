...
/// 🐕 Та же пушистая собака со спины - ПОЛНЫЙ РАЗМЕР, ПО ЦЕНТРУ
class DogBackPainter extends CustomPainter {
  @override
  void paint(Canvas canvas, Size size) {
    ...
    // 🔹 БОЛЬШОЕ ОКРУГЛОЕ ТЕЛО (ПОЛНЫЙ РАЗМЕР, ЦЕНТР)
    canvas.drawOval(
      Rect.fromCenter(
    ...
    // УХО 2
    final ear2 = Path()
      ..moveTo(size.width * 0.70, size.height * 0.24)
      ..quadraticBezierTo(
        size.width * 0.85,
        size.height * 0.18,
        size.width * 0.88,
    ...
    for (int i = 0; i < 3; i++) {
      canvas.drawLine(
        Offset(size.width * (0.17 - i * 0.03), size.height * (0.30 + i * 0.08)),
        Offset(size.width * (0.18 - i * 0.03), size.height * (0.35 + i * 0.08)),
        Paint()
          ..color = Colors.amber[700]!
          ..strokeWidth = 0.8,
      );
      canvas.drawLine(
        Offset(size.width * (0.83 + i * 0.03), size.height * (0.30 + i * 0.08)),
        Offset(size.width * (0.82 + i * 0.03), size.height * (0.35 + i * 0.08)),
        Paint()
          ..color = Colors.amber[700]!
          ..strokeWidth = 0.8,
      );
    ...
    // 🔹 ПУШИСТЫЙ ХВОСТ (ГЛАВНАЯ ФИШКА ГОЛДЕНА!)
    final tail = Path()
      ..moveTo(size.width * 0.70, size.height * 0.33)
      ..quadraticBezierTo(
        size.width * 0.90,
        size.height * 0.05,
        size.width * 1.10,
    ...
    for (int i = 0; i < 5; i++) {
      ...
      final waveX = size.width * (0.82 + i * 0.05);
  ...
  @override
  bool shouldRepaint(DogBackPainter oldDelegate) => false;
}
